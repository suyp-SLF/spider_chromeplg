...
{
	if(location.host == 'kdhrtrain.kingdee.com'){
		const scriptws = document.createElement('script')
		scriptws.type = 'text/javascript'
		scriptws.src = './function.js'
		scriptws.setAttribute('defer', 'defer')
		document.getElementsByTagName('head')[0].appendChild(scriptws);
...
chrome.runtime.onMessage.addListener(function(request, sender, sendResponse)
{
	...
	} else if ("questionin" == request.cmd){
		questionin(request.url);
	} else if ("questionsearch" == request.cmd){
		questionsearch(request.url);
	} else if ("resulton" == request.cmd){
		resulton();
	} else if ("resultoff" == request.cmd){
		resultoff();
	} else if ("testserver" == request.cmd){
		testserver(request.url);
	} else {
...
function tip(info) {
	...
	ele.style.top = tipCount * 70 + 60 + 'px';
